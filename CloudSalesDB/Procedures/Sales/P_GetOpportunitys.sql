...
GO
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'P_GetOpportunitys')
BEGIN
...
GO
/***********************************************************
过程名称： P_GetOpportunitys
功能描述： 获取机会订单列表
参数说明：	 
编写日期： 2015/11/13
程序作者： Allen
调试记录： exec P_GetOpportunitys 
************************************************************/
CREATE PROCEDURE [dbo].[P_GetOpportunitys]
	@SearchType int,
	@TypeID nvarchar(64)='',
	@StageID nvarchar(64)='',
	@Status int=-1,
...
AS
	...
	select @tableName='Opportunity o join Customer cus on o.CustomerID=cus.CustomerID ',
	...
	if(@StageID<>'')
	begin
		set @condition +=' and o.StageID = '''+@StageID+''''
	end
	...
	if(@keyWords <> '')
	begin
		set @condition +=' and (o.OpportunityCode like ''%'+@keyWords+'%'' or cus.Name like ''%'+@keyWords+'%'' or o.PersonName like ''%'+@keyWords+'%'')'
	end
