...
--存在关联数据
if exists(select c.AutoID from Category c join CategoryAttr a on c.CategoryID=a.CategoryID where c.ClientID=@ClientID and c.Status<>9 and a.AttrID=@AttrID and a.Status<>9)
begin
	set @Result=10002
	rollback tran
	return
