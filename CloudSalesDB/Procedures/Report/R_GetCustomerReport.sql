...
CREATE PROCEDURE [dbo].[R_GetCustomerReport]
	@Type int=1,--1:按地区；2、按行业；3、按规模 ;4.按标签
...
begin
	set @SqlText =' select b.ColorName as name, COUNT(cm.Name) as value from Customer as cm '
	set @SqlText +='  join CustomerColor b on cm.Mark=b.ColorID and cm.ClientID=b.ClientID'
	set @SqlText +='  where cm.Status <> 9 and b.Status <> 9 and cm.ClientID = '''+@ClientID+''''
	...
	set @SqlText +=' group by b.ColorName'
end
exec(@SqlText)
