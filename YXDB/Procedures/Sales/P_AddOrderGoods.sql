...
declare @GoodsID nvarchar(64),@Status int,@GoodsDetailID nvarchar(64),@Price decimal(18,4),
@DetailID nvarchar(64),@TotalQuantity decimal(18,4),@TotalMoney decimal(18,4)

select @GoodsID=GoodsID,@Status=Status,@Price=FinalPrice from Orders where OrderID=@OriginalID
...
begin
	...
	INSERT INTO GoodsDetail(GoodsDetailID,GoodsID,[SaleAttr],[AttrValue],[SaleAttrValue],[Price] ,[Description],[CreateUserID] ,[ClientID])
				VALUES(@DetailID,@GoodsID,@AttrList,@ValueList,@AttrValueList,@Price,@Description,@OperateID,@ClientID);
end


if exists(select AutoID from OrderGoods where GoodsDetailID=@DetailID and OrderID=@OriginalID)
begin
	Update OrderGoods set Quantity=Quantity+@Quantity where GoodsDetailID=@DetailID and OrderID=@OrderID
end
else
begin
