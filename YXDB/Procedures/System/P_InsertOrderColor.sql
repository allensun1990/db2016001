...
as
set @result=0
if((select COUNT(1) from OrderColor where Status<>9 and  ClientID=@ClientID and AgentID=@AgentID and ColorValue=@ColorValue )>0)
begin
set @result=-1
end
else
begin
	declare @ColorID int
	select @ColorID=isnull(MAX(ColorID),0)+1 from OrderColor where ClientID=@ClientID and AgentID=@AgentID
